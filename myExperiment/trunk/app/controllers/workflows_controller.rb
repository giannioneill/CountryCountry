...
class WorkflowsController < ApplicationController
  ...
  def run_flow(host,port,user,pass,audioFiles)
    #Running a workflow consists of two distinct steps
    # 1) uploading the workflow to the Meandre server
    # 2) executing the flow with the various input arguments
    pwdstring = user+':'+pass
    url = "http://#{host}:#{port}/"
    ...
    begin
      ...
      flash[:error] = 'Could not connect to Meandre server'
  ...
  # GET /workflows/1/run
  def run
    if params[:meandre_user] == nil || params[:meandre_password] ==nil
     flash[:error] = 'Username and Password required' 
     redirect_to :action=>'show'
     return
    end
    if params[:meandre_host] == nil || params[:meandre_port] == nil
    ...
    begin
      ...
      flash[:error] = 'Cannot find that collection.'
      redirect_to :action=>'show'
      return
    end
    
    collection_parser = CollectionParser.new(curl.body_str)

    signals = collection_parser.find_signals()
    logger.debug "LIST OF SIGNALS..."
    signals.each {|s| logger.debug("SIGNAL: "+s)}

    spawn do
      run_flow(params[:meandre_host], 
        params[:meandre_port],
        params[:meandre_user],
        params[:meandre_password],
        signals)
    end
    flash[:notice] = 'The workflow is now executing.'
    redirect_to :action=>'show'
  end
